...
public interface IQueryProvider<TEntity> where TEntity : class
{
    ...
    Expression<Func<TEntity, bool>> ByEntityId(string nameEntityId, Guid entityId,
        bool isEntityForeignKey = false);

    Expression<Func<TEntity, bool>> ByPropertyName(string propertyName, string propertyValue,
